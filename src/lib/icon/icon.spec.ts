...
fdescribe('MatIcon', () => {
    ...
    xit('should apply class based on color attribute', () => {
        ...
        expect(sortedClassNames(matIconElement)).toEqual(['apto-icon', 'mat-primary', 'material-icons']);
    });
...
describe('MatIcon without HttpClientModule', () => {
  ...
  it('should throw an error when trying to load a remote icon', async() => {
    const expectedError = wrappedErrorMessage(getAptoIconNoHttpProviderError());
...
@Component({template: `<apto-icon [color]="iconColor" [icon]="iconName"></apto-icon>`})
class IconWithColor {
  iconName = '';
  iconColor = 'primary';
}
...
@Component({template: '<apto-icon [icon]="face" aria-hidden="false"></apto-icon>'})
